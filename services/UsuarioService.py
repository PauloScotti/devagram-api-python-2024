...
class UsuarioService:
    async def registrar_usuario(self, usuario: UsuarioCriarModel, caminho_foto):
        try:
            usuario_encotrado = await usuarioRepository.buscar_usuario_por_email(usuario.email)

            if usuario_encotrado:
                return {
                ...
                try:
                    url_foto = aswProvider.upload_arquivo_s3(f'fotos-perfil/{novo_usuario["id"]}.png', caminho_foto)
    ...
    async def buscar_usuario(self, id: str):
        try:
            usuario_encotrado = await usuarioRepository.buscar_usuario_logado(id)

            if usuario_encotrado:
                return {
                    "mensagem": f"Usuário encontrado.",
                    "dados": usuario_encotrado,
                    "status": 200
                }
            else:
                return {
        ...
        try:
            ...
            if usuario_encotrado:
                ...
                try:
                    ...
                    url_foto = aswProvider.upload_arquivo_s3(f'fotos-perfil/{id}.png', caminho_foto)
